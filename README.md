...
프로젝트 폴더 구조 예시 (monorepo 방식 추천 – Vercel에서 편함)
...
Atlas 대시보드 → 새 클러스터 생성 (무료 M0 tier)
Database Access → 새 사용자 생성 (username/password 기억)
Network Access → IP 주소 0.0.0.0/0 허용 (테스트용 – 나중에 제한 가능)
Connect → Drivers → Node.js 선택 → 연결 문자열 복사
예: mongodb+srv://<user>:<pass>@cluster0.abcde.mongodb.net/todoDB?retryWrites=true&w=majority
Step 2: 백엔드 만들기
backend 폴더로 이동 → npm init -y
패키지 설치:
...
npm run dev로 로컬 테스트 (Postman으로 API 확인)
Vercel 배포용 준비 (중요!)
...
Tailwind 설정
src/App.jsx에 Todo UI 구현 (axios로 백엔드 호출)
...
전체 프로젝트 push
https://vercel.com → New Project → GitHub Import
프로젝트 루트 설정 (frontend 또는 전체 monorepo)
Environment Variables: MONGODB_URI 추가
Deploy → 성공 시 URL 발급 (예: https://todo-app-학번.vercel.app)
4. 자주 발생하는 에러 & 해결
